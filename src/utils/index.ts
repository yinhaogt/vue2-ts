import { SlideBackground } from '@/types/slide'

// 将页面背景数据转换为css样式
export function slideBackgroundStyle(background: SlideBackground) {
  if (!background) return { backgroundColor: '#fff' }

  const { type, color, image, imageSize, gradientColor, gradientRotate, gradientType } = background
